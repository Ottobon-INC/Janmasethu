...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // =========================
  // PROXY ROUTES FOR EXTERNAL AUTH API
  // =========================
  const EXTERNAL_API_URL = process.env.EXTERNAL_API_URL || 'https://uncollectively-unfutile-deandrea.ngrok-free.dev';

  // Proxy: User Login
  app.post("/api/proxy/user/login", async (req, res) => {
    try {
      const response = await fetch(`${EXTERNAL_API_URL}/user/login`, {
        method: 'POST',
        headers: {
  ...
  app.post("/api/proxy/user/register", async (req, res) => {
    try {
      const response = await fetch(`${EXTERNAL_API_URL}/user/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': 'true'
  ...
  app.post("/api/proxy/sakhi/chat", async (req, res) => {
    try {
      const response = await fetch(`${EXTERNAL_API_URL}/sakhi/chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': 'true'
  ...
  app.post("/api/proxy/onboarding/step", async (req, res) => {
    try {
      const response = await fetch(`${EXTERNAL_API_URL}/onboarding/step`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': 'true'
      ...
      res.status(500).json({ error: "Failed to connect to onboarding service" });
    }
  });
  ...
  app.post("/api/proxy/onboarding/complete", async (req, res) => {
    try {
      const response = await fetch(`${EXTERNAL_API_URL}/onboarding/complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': 'true'
